...
### 2026-09-23 전체 복구 트리 무결성 검사

사용자가 원본 201,549개 복구 완료를 알린 뒤 `samples.csv` 전체를 기준으로 길이와 SHA-256을
정적 검증했다. 파일명 누락과 0바이트는 0이지만, 201,498개만 완전 일치했고 51개가 실패했다
(길이 불일치 41, 기대 길이에서 SHA-256 불일치 10). 51개 모두 raster index의 악성 train/val
표본이며 과거 누락 180개와 겹치지 않는다. 따라서 P2 무결성 게이트는 여전히 실패다.

현재 `D:\secure-malware-data\pe-ml-enc.7z`은 목록상 201,371개 파일만 포함하고 현재의 잘못된
표본도 포함하므로 신뢰 가능한 전체 복구원으로 사용하지 않았다. 추출·덮어쓰기·삭제는 하지 않았다.
정확한 51개 복구 목록은
`D:\secure-malware-data\psa\audit\source_integrity_20260923\recovery_manifest_51.csv`
(SHA256 `d3b1f6e887b9ac792b83c1958707a805332800e1dee39f24dd717412cdd9de37`)이다.
